...
class Sqll {
    constructor(config){ 
        ...
        this.escape = this.connection.escape;
        ...
        let from = tables.map(
            (value, i) => {
                if(i > 0){
                    ...
                    let extkey = obj[value].exkey ? obj[value].exkey : 'id';
                    if(obj[value]._key)
                        return `JOIN ${value} ON ${value}.${extkey} = ${obj[value]._key}`
                    return `JOIN ${value} ON ${value}.${extkey} = ${chave_anterior}.${obj[value].key}`
                } else {
                    return value
                }
            }
